...
// ---- Branded client email (booking, intake, reschedule, cancel) -------------
// One template so every transactional client email reads the same: brand stripe,
// logo, a heading, a greeting, an optional details table, optional buttons, an
// optional note, and a footer. Content is structured, not one wall of text.

export const CLIENT_LOGO_CID = "tifec-client-logo";

export interface ClientEmailArgs {
  heading: string;
  greetingName?: string;                       // "Maria" -> "Hi Maria,"
  intro?: string;                              // one lead paragraph
  rows?: { label: string; value: string }[];   // a details table (Service, When, ...)
  buttons?: { label: string; url: string }[];  // call-to-action links
  note?: string;                               // subtle callout
  outro?: string;                              // closing paragraph before sign-off
  logoCid?: string;
}

/** Subject-less text + HTML for a branded client email (exported for previews). */
export function buildClientEmail(a: ClientEmailArgs): { text: string; html: string } {
  const greet = a.greetingName ? `Hi ${a.greetingName},` : "Hello,";

  // plain-text fallback
  const t: string[] = [greet, ""];
  if (a.intro) t.push(a.intro, "");
  if (a.rows?.length) { for (const r of a.rows) t.push(`${r.label}: ${r.value}`); t.push(""); }
  if (a.buttons?.length) { for (const b of a.buttons) t.push(`${b.label}:`, b.url, ""); }
  if (a.note) t.push(a.note, "");
  if (a.outro) t.push(a.outro, "");
